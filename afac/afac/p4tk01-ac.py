...
extensions = [".odr", ".mesh"]
keywords = ["emis", "weed", "glue", "gloo", "emmis", "nite", "emiss", "pedpost", "ropetyre", "glu", "seast", "seabed", "puerto", "pathpatch", "_nl_", "detail", "_det_", "_glue_", "night", "weeds", "em", "_glue", "canopys", "_det", "_nl"]
...
# Define a function to delete files recursively
def delete_files_recursive(directory):
    for root, dirs, files in os.walk(directory):
        for file in files:
            if any(file.endswith(x) for x in delete_types):
                file_path = os.path.join(root, file)
                os.remove(file_path)


cwd = os.getcwd()
delete_files_recursive(cwd)
...
def move_dae_files():
    ...
    destination_dir = os.path.join(parent_dir, 'ac-test', 'model')
    ...
    if not dae_files:
        print("No .dae files found in the current working directory.")
        return

    # Move .dae files to ac-test/model directory
    for dae_file in dae_files:
        source_path = os.path.join(cwd, dae_file)
        shutil.move(source_path, destination_dir)

    # Create a folder with the same name as the destination directory
    zone_name = os.path.basename(os.path.normpath(destination_dir))
    zone_dir = os.path.join(destination_dir, zone_name)
    os.makedirs(zone_dir, exist_ok=True)

    # Move the .dae files to the zone folder
    for dae_file in dae_files:
        source_path = os.path.join(destination_dir, dae_file)
        target_path = os.path.join(zone_dir, dae_file)
        shutil.move(source_path, target_path)

    print("Files moved successfully.")

# Call the function to execute the moving process
move_dae_files()
