...
for dir_entry in os.scandir("."):
    if dir_entry.is_file() and dir_entry.name.endswith(".odr"):
        ...
        # we try except here to dodge models that I call "quasi", which are .ydr that are shadows or other engine parameter files wrapped in a .ydr. Fastest way around
        except IndexError:
            print(f"Index error occurred, ignore, this model: {dir_entry.name} is likely a quasi-model. Skipping this file.")
            continue
...
# execute batch.py as a separate process
...
# execute batch.py as a separate process
subprocess.call(["python", "citizen.py"])